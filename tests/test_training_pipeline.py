...
assert tr3 >= total * 0.98, (
    f"Phase 3 must unfreeze nearly all params (BN stays frozen), got {100 * tr3 / total:.1f}%"
)
...
ok(
    f"optimal_threshold={metrics['optimal_threshold']:.4f}  youden_j={metrics['youden_j']:.4f}"
)
...
ok(
    f"best_value={es.best_value:.3f}  best_epoch={es.best_epoch}  counter={es.counter}  stopped={stopped}"
)
...
assert aug_result["image"].shape == (224, 224, 3), (
    "Train transform output shape mismatch"
)
...
ok(
    f"Checkpoint loaded: epoch={state['epoch']}, auc={state['current_metrics'].get('auc_roc', 0):.2f}"
)

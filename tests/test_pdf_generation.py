...
class TestMedicalReportDataConstruction:
    ...
    @pytest.mark.parametrize("prediction,expected_substr", [
        ("ALL+",    "ALL"),
        ("Healthy", "HEALTHY"),
    ])
...
class TestGenerateFile:
    ...
    def test_different_images_produce_different_files(self, tmp_path):
        ...
        data1 = MedicalReportData(
            original_image=_make_image(), heatmap_overlay=_make_image(),
            prediction="ALL+", probability=0.95, confidence=0.90,
            risk_level="LOW", clinical_note="test",
            report_id="RPT-A0000001",
        )
        data2 = MedicalReportData(
            original_image=_make_image()*0, heatmap_overlay=_make_image()*0,
            prediction="Healthy", probability=0.05, confidence=0.92,
            risk_level="LOW", clinical_note="test",

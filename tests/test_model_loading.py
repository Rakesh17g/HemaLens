...
class TestBuildModel:
    ...
    def test_model_has_features_attr(self, model):
        assert hasattr(model, "features"), (
            "Model must have a .features attribute for GradCAM"
        )
    ...
    def test_total_parameter_count(self, model):
        ...
        assert 4_000_000 < total < 7_000_000, f"Expected ~5.3M params, got {total:,}"
...
class TestCheckpointLoading:
    def test_load_wrapped_checkpoint(self, tmp_checkpoint, device):
        ...
        loaded_model, _loaded_device = load_model(str(tmp_checkpoint), str(device))
    ...
    def test_missing_checkpoint_returns_none(self, device):
        ...
        model_out, _device_out = load_model("/non/existent/path.pth", str(device))
    ...
    def test_loaded_model_produces_same_output(
        self, model, tmp_checkpoint, rand_tensor, device
    ):

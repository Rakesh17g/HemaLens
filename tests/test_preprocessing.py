...
class TestPreprocessOutputShapes:
    ...
    def test_tensor_shape_default_size(self, rand_pil):
        tensor, orig_rgb = preprocess(rand_pil, target_size=224)
    ...
    def test_tiny_input_resizes_correctly(self):
        tiny = make_pil(16, 16)
        tensor, orig = preprocess(tiny, target_size=224)
        assert tensor.shape == (1, 3, 224, 224)
...
class TestPreprocessNormalisation:
    ...
    def test_different_images_produce_different_tensors(self):
        pil_a = make_pil(224, 224); rng = np.random.default_rng(99)
        ...
        assert not torch.allclose(t_a, t_b), "Two distinct images gave identical tensors"

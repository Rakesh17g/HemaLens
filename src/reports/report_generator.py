...
class _Badge(Flowable):
    ...
    def draw(self):
        ...
        c.drawCentredString(
            self.width / 2, self.height / 2 - self.font_sz * 0.36, self.text
        )
...
class MedicalReportGenerator:
    ...
    def generate(
        ...
        doc.build(
            story,
            onFirstPage=self._page_decorator(data),
    ...
    def generate_bytes(
        ...
        """
        buf = io.BytesIO()
        doc = self._build_doc(buf, data)  # type: ignore
        story = self._build_story(data)
        doc.build(
            story,
            onFirstPage=self._page_decorator(data),
    ...
    def _page_decorator(self, data: MedicalReportData):
        ...
        def on_page(canv: canvas.Canvas, doc) -> None:
            ...
            canv.drawRightString(
                right_x, page_h - 1.35 * cm, f"Report ID: {data.report_id}"
            )
            canv.setFont("Helvetica", 8)
            canv.setFillColor(colors.HexColor("#CBD5E1"))
            canv.drawRightString(
                right_x, page_h - 1.85 * cm, f"Date: {data.datetime_str}"
            )
            canv.drawRightString(
                right_x,
                page_h - 2.25 * cm,
            ...
            # Page number
            canv.drawRightString(page_w - 1.5 * cm, 0.6 * cm, f"Page {doc.page}")
    ...
    def _build_story(self, data: MedicalReportData) -> list:
        ...
        risk_label = {
            ...
            "MEDIUM": "MEDIUM RISK",
            "HIGH": "HIGH RISK",
        }.get(data.risk_level, data.risk_level)
        ...
        def _bar_row(label: str, value: float, color_hex: str, note: str):
            bar_full = 8.0 * cm
            bar_full * max(0.0, min(1.0, value))
            bar_cell = (
            ...
            return [
                ...
                Paragraph(
                    f"<b><font color='{color_hex}'>{value:.3f}</font></b>",
                    styles["tbl_val"],
                ),
                Paragraph(
                    f"<font color='#64748B' size='7.5'>{note}</font>",
                    styles["tbl_note"],
                ),
            ]
        ...
        story.append(
            HRFlowable(
                width="100%", thickness=0.5, color=_C.DIVIDER, spaceAfter=0.15 * cm
            )
        )

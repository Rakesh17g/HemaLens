...
def plot_metrics_dashboard(
    ...
    for ax, (name, val, color) in zip(axes, metrics):
        ...
        ax.plot(r * np.cos(theta_bg) + 0.5, r * np.sin(theta_bg) + 0.55,
                color=PALETTE["border"], linewidth=6, solid_capstyle="round")
        if val_clipped > 0.01:
            ax.plot(r * np.cos(theta) + 0.5, r * np.sin(theta) + 0.55,
                    color=color, linewidth=6, solid_capstyle="round")

        # Value text
        ax.text(0.5, 0.52, f"{val:.4f}", ha="center", va="center",
                color=color, fontsize=16, fontweight="bold",
                transform=ax.transAxes)

        # Metric name
        ax.text(0.5, 0.18, name, ha="center", va="center",
    ...
    plt.tight_layout(pad=1.2, rect=[0, 0.03, 1, 0.96])
...
def plot_roc_curve(
    ...
    _setup_ax(ax, xlabel="False Positive Rate (1 − Specificity)",
    ...
    # ROC line
    ax.plot(fpr, tpr, color=PALETTE["accent1"], linewidth=2.5,
            label=f"EfficientNet-B0  (AUC = {auc:.4f})")

    # Diagonal (random)
    ax.plot([0, 1], [0, 1], color=PALETTE["subtext"], linewidth=1,
            linestyle="--", label="Random Classifier (AUC = 0.50)")

    # Youden's J operating point
    ax.scatter(fpr[idx], tpr[idx], s=120, color=PALETTE["accent3"],
               zorder=5, label=f"Youden's J Point  (t={opt_t:.3f})")
    ax.annotate(
        f" Sens={tpr[idx]:.3f}\n Spec={1-fpr[idx]:.3f}",
        xy=(fpr[idx], tpr[idx]),
        xytext=(fpr[idx] + 0.05, tpr[idx] - 0.08),
        color=PALETTE["accent3"], fontsize=8,
        arrowprops=dict(arrowstyle="->", color=PALETTE["accent3"], lw=1.0),
    )

    # Formatting
    ax.set_xlim(-0.02, 1.02)
    ax.set_ylim(-0.02, 1.02)
    legend = ax.legend(loc="lower right", facecolor=PALETTE["surface"],
                       edgecolor=PALETTE["border"], labelcolor=PALETTE["text"],
                       fontsize=8.5)

    # AUC badge
    ax.text(0.97, 0.08, f"AUC\n{auc:.4f}", ha="right", va="bottom",
            color=PALETTE["accent1"], fontsize=14, fontweight="bold",
            transform=ax.transAxes)
...
def plot_pr_curve(
    ...
    ax.plot(rec, prec, color=PALETTE["accent2"], linewidth=2.5,
            label=f"EfficientNet-B0  (AUPRC = {auprc:.4f})")
    ax.axhline(prevalence, color=PALETTE["subtext"], linewidth=1,
               linestyle="--", label=f"Chance (prevalence = {prevalence:.3f})")
    ax.scatter(op_rec, op_prec, s=120, color=PALETTE["accent3"], zorder=5,
               label=f"Operating Point  (t={results['applied_threshold']:.3f})")

    ax.set_xlim(-0.02, 1.02)
    ax.set_ylim(0.0, 1.05)
    ax.legend(loc="lower left", facecolor=PALETTE["surface"],
              edgecolor=PALETTE["border"], labelcolor=PALETTE["text"],
              fontsize=8.5)
    ax.text(0.97, 0.97, f"AUPRC\n{auprc:.4f}", ha="right", va="top",
            color=PALETTE["accent2"], fontsize=14, fontweight="bold",
            transform=ax.transAxes)
...
def plot_confusion_matrix(
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax.text(j, i - 0.18, str(count),
                    ha="center", va="center", fontsize=22, fontweight="bold",
                    color=txt_col)
            ax.text(j, i + 0.10, f"{pct:.1f}% of total",
    ...
    ax.set_yticklabels(names, color=PALETTE["text"], fontsize=10,
    ...
    fig.text(0.5, 0.01,
             ...
             ha="center", color=PALETTE["subtext"], fontsize=8)
...
def plot_classification_report(
    ...
    for r_idx, row in enumerate(all_rows):
        ...
        bg_col = PALETTE["surface"] if is_header else (
            PALETTE["card"] if r_idx % 2 == 0 else PALETTE["bg"]
        )
        if not is_empty:
            rect = mpatches.FancyBboxPatch(
                (0.01, y - row_h * 0.85), 0.98, row_h * 0.9,
                transform=ax.transAxes, boxstyle="round,pad=0.005",
                facecolor=bg_col, edgecolor=PALETTE["border"], linewidth=0.5
            )
            ax.add_patch(rect)
        ...
        for c_idx, (cell, xw) in enumerate(zip(row, x_starts)):
            ...
            ax.text(xw + col_widths[c_idx] / 2, y - row_h * 0.35,
                    cell, ha="center", va="center",
                    color=color, fontsize=9.5, fontweight=fw,
                    transform=ax.transAxes)
...
def plot_threshold_analysis(
    ...
    ax.axvline(opt_t, color=PALETTE["accent3"], linewidth=1.5,
               linestyle="--", alpha=0.8)
    ax.text(opt_t + 0.01, 0.97, f"Optimal t={opt_t:.3f}",
            color=PALETTE["accent3"], fontsize=8, va="top")

    # Applied threshold marker
    applied_t = results["applied_threshold"]
    if abs(applied_t - opt_t) > 0.01:
        ax.axvline(applied_t, color=PALETTE["accent2"], linewidth=1.0,
                   linestyle=":", alpha=0.7)
        ax.text(applied_t + 0.01, 0.88, f"Applied t={applied_t:.3f}",
                color=PALETTE["accent2"], fontsize=7.5, va="top")

    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1.05)
    legend = ax.legend(loc="lower center", ncol=4,
...
def plot_probability_distribution(
    ...
    _setup_ax(ax, xlabel="Predicted Probability (P[ALL+])",
    ...
    ax.hist(y_prob[neg_idx], bins=bins, color=PALETTE["negative"],
            alpha=0.75, label=f"Healthy (0)  n={neg_idx.sum()}",
            edgecolor=PALETTE["bg"], linewidth=0.4)
    ax.hist(y_prob[pos_idx], bins=bins, color=PALETTE["positive"],
            alpha=0.75, label=f"ALL+ (1)     n={pos_idx.sum()}",
            edgecolor=PALETTE["bg"], linewidth=0.4)

    ax.axvline(t, color=PALETTE["accent3"], linewidth=2,
               linestyle="--", label=f"Threshold  t={t:.3f}")
    ...
    legend = ax.legend(facecolor=PALETTE["surface"],
                       edgecolor=PALETTE["border"],
...
def plot_evaluation_report(
    ...
    ax_roc.plot(fpr, tpr, color=PALETTE["accent1"], lw=2,
                label=f"AUC={auc:.4f}")
    ...
    ax_roc.legend(loc="lower right", facecolor=PALETTE["surface"],
                  edgecolor=PALETTE["border"], labelcolor=PALETTE["text"],
                  fontsize=7.5)
    ...
    ax_pr.plot(rec, prec, color=PALETTE["accent2"], lw=2,
               label=f"AUPRC={auprc:.4f}")
    ax_pr.axhline(prev, color=PALETTE["subtext"], lw=0.8, linestyle="--")
    ax_pr.scatter(results["recall"], results["precision"],
                  s=80, color=PALETTE["accent3"], zorder=5)
    ax_pr.set_xlim(-0.02, 1.02); ax_pr.set_ylim(0, 1.05)
    ax_pr.legend(loc="lower left", facecolor=PALETTE["surface"],
                 edgecolor=PALETTE["border"], labelcolor=PALETTE["text"],
                 fontsize=7.5)
    ...
    ax_sc.set_title("Key Metrics", color=PALETTE["text"],
    ...
    for i, (name, val, color) in enumerate(scorecard):
        ...
        ax_sc.barh([y], [bw], height=0.08, left=0.35,
                   color=color, alpha=0.7, transform=ax_sc.transAxes)
        ax_sc.text(0.02, y, name, color=PALETTE["subtext"], fontsize=8,
                   va="center", transform=ax_sc.transAxes)
        ax_sc.text(0.93, y, f"{val:.4f}", color=color, fontsize=8.5,
    ...
    _setup_ax(ax_th, "Metric vs. Decision Threshold",
              "Threshold", "Value")
    ...
    ax_th.plot(thresholds, sens_vals, color=PALETTE["positive"],  lw=2, label="Sensitivity")
    ax_th.plot(thresholds, spec_vals, color=PALETTE["negative"],  lw=2, label="Specificity")
    ax_th.axvline(opt, color=PALETTE["accent3"], lw=1.5, linestyle="--",
                  label=f"Youden's J (t={opt:.3f})")
    ax_th.set_xlim(0, 1); ax_th.set_ylim(0, 1.05)
    ax_th.legend(facecolor=PALETTE["surface"], edgecolor=PALETTE["border"],
                 labelcolor=PALETTE["text"], fontsize=8, loc="lower center",
                 ncol=4)
...
def generate_all_plots(
    ...
    plot_evaluation_report(results, report_path,
                           model_name=model_name, split=split.capitalize())

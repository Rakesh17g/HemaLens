...
def _ax_base(ax: plt.Axes, title: str = "", xlabel: str = "",
...
def plot_confidence_gauge(
    ...
    _fig_base.__wrapped__ = None  # avoid recursion quirk
    fig.patch.set_facecolor(PALETTE["bg"])
    fig.suptitle("Confidence Estimation — Single Sample",
                 color=PALETTE["text"], fontsize=12, fontweight="bold", y=0.98)
    ...
    def _gauge(ax, value, color, title, subtitle):
        ...
        cx, cy = 0.5, 0.48
        ax.plot(
            cx + r * np.cos(theta_bg), cy + r * np.sin(theta_bg),
            color=PALETTE["border"], linewidth=10, solid_capstyle="round",
        )
        ...
        if value > 0.005:
            ax.plot(
                cx + r * np.cos(theta_v), cy + r * np.sin(theta_v),
        ...
        ax.scatter(
            [cx + r * np.cos(angle)],
            [cy + r * np.sin(angle)],
            s=140, color=color, zorder=6, edgecolors=PALETTE["bg"], linewidths=2,
        )

        # Value text
        ax.text(cx, cy, f"{value:.3f}", ha="center", va="center",
                color=color, fontsize=26, fontweight="bold",
                transform=ax.transAxes)
        ax.text(cx, cy - 0.14, f"{value*100:.1f}%", ha="center", va="center",
                color=PALETTE["subtext"], fontsize=11, transform=ax.transAxes)

        # Title and subtitle
        ax.text(cx, 0.90, title, ha="center", va="top",
                color=PALETTE["text"], fontsize=11, fontweight="bold",
                transform=ax.transAxes)
        ax.text(cx, 0.10, subtitle, ha="center", va="bottom",
                color=PALETTE["subtext"], fontsize=8.5, transform=ax.transAxes,
                wrap=True)
    ...
    fig.text(
        ...
        + (f"MC-Dropout={result.mcdrop_score:.3f}  (T={result.mcdrop_passes})"
           if result.mcdrop_passes > 0 else "MC-Dropout=disabled"),
        ha="center", color=r_col, fontsize=9, fontweight="bold",
    )
...
def plot_confidence_vs_probability(
    ...
    fig.patch.set_facecolor(PALETTE["bg"])
    _ax_base(ax, "Confidence vs Prediction Probability",
             "Prediction Probability  p", "Confidence Score  c")
    ...
    for risk, col in RISK_COLORS.items():
        ...
        if not any(mask):
            continue
        ax.scatter(
            probs[mask], confs[mask],
            c=col, s=55, alpha=0.75, edgecolors=PALETTE["bg"],
            linewidths=0.4, label=f"{risk} risk (n={sum(mask)})",
            zorder=3,
        )

    # Threshold lines
    ax.axvline(0.5, color=PALETTE["accent3"], lw=1.2,
               linestyle="--", alpha=0.5, label="p=0.50")
    ax.axhline(0.80, color=PALETTE["low"],    lw=1.0,
               linestyle=":", alpha=0.6, label="c=0.80 (LOW/MEDIUM)")
    ax.axhline(0.55, color=PALETTE["medium"], lw=1.0,
               linestyle=":", alpha=0.6, label="c=0.55 (MEDIUM/HIGH)")
...
def plot_risk_distribution(
    ...
    fig.patch.set_facecolor(PALETTE["bg"])
    fig.suptitle("Risk Level Distribution", color=PALETTE["text"],
    ...
    bars = ax1.bar(levels, total_counts, color=colors, edgecolor=PALETTE["bg"],
    ...
    for i, (pred_cls, mc) in enumerate(
        ...
        b = ax2.bar(x + (i - 0.5) * bw, vals, bw,
                    color=mc, alpha=0.80, edgecolor=PALETTE["bg"],
                    linewidth=0.6, label=pred_cls)
    ax2.set_xticks(x)
    ax2.set_xticklabels(levels, color=PALETTE["text"], fontsize=9)
    ax2.legend(facecolor=PALETTE["surface"], edgecolor=PALETTE["border"],
               labelcolor=PALETTE["text"], fontsize=8.5)

    # ── Pie chart ──────────────────────────────────────────────────────────
    ax3 = fig.add_subplot(gs[2])
    ax3.set_facecolor(PALETTE["card"])
    ax3.set_title("Risk Share", color=PALETTE["text"],
                  fontsize=9, fontweight="bold", pad=6)
    non_zero  = [(c, lv, col) for c, lv, col in zip(total_counts, levels, colors) if c > 0]
    if non_zero:
...
def plot_confidence_histogram(
    ...
    fig.patch.set_facecolor(PALETTE["bg"])
    _ax_base(ax, "Confidence Score Distribution by Predicted Class",
             "Confidence Score  c", "Count")

    bins = np.linspace(0, 1, 31)
    if confs_neg:
        ax.hist(confs_neg, bins=bins, color=PALETTE["negative"], alpha=0.72,
                label=f"Healthy  (n={len(confs_neg)})",
                edgecolor=PALETTE["bg"], linewidth=0.4)
    if confs_pos:
        ax.hist(confs_pos, bins=bins, color=PALETTE["positive"], alpha=0.72,
    ...
    for boundary, label, col in [
        ...
        ax.text(boundary + 0.01, ax.get_ylim()[1] * 0.97, label,
                color=col, fontsize=7.5, va="top")

    # Risk zone shading
    ax.axvspan(0,    0.55, alpha=0.07, color=PALETTE["high"],   label="HIGH risk zone")
    ax.axvspan(0.55, 0.80, alpha=0.07, color=PALETTE["medium"], label="MEDIUM risk zone")
    ...
    ax.legend(facecolor=PALETTE["surface"], edgecolor=PALETTE["border"],
              labelcolor=PALETTE["text"], fontsize=8.5, ncol=2,
              loc="upper left")
...
def plot_confidence_reliability(
    ...
    for lo, hi in zip(bin_edges[:-1], bin_edges[1:]):
        ...
        if mask.sum() == 0:
            mean_conf_per_bin.append(None)
            mean_acc_per_bin.append(None)
            bin_counts.append(0)
        else:
    ...
    fig.patch.set_facecolor(PALETTE["bg"])
    fig.suptitle("Confidence Reliability Diagram",
                 color=PALETTE["text"], fontsize=12, fontweight="bold", y=0.98)

    # ── Left: reliability diagram ─────────────────────────────────────────
    _ax_base(ax1, "Mean Confidence vs Accuracy per Bin",
             "Mean Confidence in Bin", "Fraction Correct")

    valid_x = [x for x in mean_conf_per_bin if x is not None]
    valid_y = [y for y in mean_acc_per_bin  if y is not None]

    # Perfect calibration diagonal
    ax1.plot([0, 1], [0, 1], "--", color=PALETTE["subtext"],
             linewidth=1.2, label="Perfect calibration", alpha=0.6)
    # Gap shading
    for xv, yv in zip(valid_x, valid_y):
        ax1.fill_between([xv], [xv], [yv],
                         color=PALETTE["accent3"], alpha=0.25)
    # Reliability plot
    ax1.plot(valid_x, valid_y, "o-",
             color=PALETTE["accent1"], linewidth=2, markersize=7,
             label="Model reliability", zorder=4)

    ax1.set_xlim(-0.02, 1.02)
    ax1.set_ylim(-0.02, 1.05)
    ax1.legend(facecolor=PALETTE["surface"], edgecolor=PALETTE["border"],
               labelcolor=PALETTE["text"], fontsize=8.5)

    # ── Right: bin count bar chart ────────────────────────────────────────
    _ax_base(ax2, "Samples per Confidence Bin", "Confidence Bin", "Count")
    bar_colors = [
        PALETTE["low"] if c >= 0.80 else
        PALETTE["medium"] if c >= 0.55 else
        PALETTE["high"]
        for c in bin_centers
    ]
    ax2.bar(bin_centers, bin_counts, width=0.09,
            color=bar_colors, edgecolor=PALETTE["bg"], linewidth=0.5)
    ax2.set_xlim(0, 1)
...
def plot_confidence_report(
    result:      Any,         # ConfidenceResult
    output_path: str,
    image_array: Optional[np.ndarray] = None,  # (H,W,3) uint8 — optional
) -> None:
    """
    ...
    if image_array is not None:
        ...
        ax_img.text(0.5, 0.5, "Cell Image\n(not provided)",
                    ha="center", va="center", color=PALETTE["subtext"],
                    fontsize=10, transform=ax_img.transAxes)
        ax_img.axis("off")
    ax_img.set_title(
        f"{pred}  |  {emoji_map[risk]} {risk} RISK",
        color=r_col, fontsize=10, fontweight="bold", pad=8
    )
    ...
    r = 0.35
    ax_prob.plot(r * np.cos(theta_bg) + 0.5, r * np.sin(theta_bg) + 0.52,
                 color=PALETTE["border"], linewidth=8, solid_capstyle="round")
    if prob > 0.005:
        ax_prob.plot(r * np.cos(theta) + 0.5, r * np.sin(theta) + 0.52,
                     color=p_col, linewidth=8, solid_capstyle="round")
    ax_prob.text(0.5, 0.50, f"{prob:.3f}", ha="center", va="center",
                 color=p_col, fontsize=20, fontweight="bold",
                 transform=ax_prob.transAxes)
    ax_prob.text(0.5, 0.18, "Prediction Probability", ha="center",
                 color=PALETTE["subtext"], fontsize=8, transform=ax_prob.transAxes)
    ax_prob.set_xlim(0, 1); ax_prob.set_ylim(0, 1); ax_prob.axis("off")
    ...
    ax_conf.plot(r * np.cos(theta_bg) + 0.5, r * np.sin(theta_bg) + 0.52,
                 color=PALETTE["border"], linewidth=8, solid_capstyle="round")
    if conf > 0.005:
        ax_conf.plot(r * np.cos(theta_c) + 0.5, r * np.sin(theta_c) + 0.52,
                     color=r_col, linewidth=8, solid_capstyle="round")
    ax_conf.text(0.5, 0.50, f"{conf:.3f}", ha="center", va="center",
                 color=r_col, fontsize=20, fontweight="bold",
                 transform=ax_conf.transAxes)
    ax_conf.text(0.5, 0.18, "Confidence Score", ha="center",
                 color=PALETTE["subtext"], fontsize=8, transform=ax_conf.transAxes)
    ax_conf.set_xlim(0, 1); ax_conf.set_ylim(0, 1); ax_conf.axis("off")
    ...
    for spine in ax_risk.spines.values():
        ...
        spine.set_linewidth(2.5)
    ax_risk.axis("off")
    ax_risk.text(0.5, 0.72, emoji_map[risk], ha="center", va="center",
                 fontsize=42, transform=ax_risk.transAxes)
    ax_risk.text(0.5, 0.48, risk + " RISK", ha="center", va="center",
                 color=r_col, fontsize=16, fontweight="bold",
                 transform=ax_risk.transAxes)
    ax_risk.text(0.5, 0.25, result.clinical_note, ha="center", va="center",
                 color=PALETTE["subtext"], fontsize=7.5,
                 transform=ax_risk.transAxes, wrap=True)

    # ── Bottom row: confidence breakdown bars ─────────────────────────────
    ax_bd = fig.add_subplot(gs[1, 1:])
    _ax_base(ax_bd, "Confidence Component Breakdown",
             "Score (0 = uncertain, 1 = certain)", "", grid=False)

    components = [
        ("Boundary Distance", result.boundary_score, PALETTE["accent1"],
         f"|p−t|/max_dist  =  |{prob:.3f}−{result.threshold:.2f}|  →  {result.boundary_score:.3f}"),
        ("Entropy Score",     result.entropy_score,  PALETTE["accent2"],
         f"1 − H(p)/ln2  |  H={result.entropy_raw:.4f}  →  {result.entropy_score:.3f}"),
    ]
    if result.mcdrop_passes > 0:
        components.append((
            f"MC Dropout (T={result.mcdrop_passes})", result.mcdrop_score, PALETTE["accent3"],
            f"1 − σ/0.25  |  σ={result.mcdrop_std:.4f}  mean={result.mcdrop_mean:.3f}  →  {result.mcdrop_score:.3f}",
        ))
    components.append(
        ("Combined Confidence", conf, r_col,
    ...
    for y, (name, val, col, annot) in zip(ys, components):
        ax_bd.barh([y], [val], height=0.7, color=col, alpha=0.85,
                   edgecolor=PALETTE["bg"], linewidth=0.5)
        ax_bd.text(-0.01, y, name, ha="right", va="center",
                   color=PALETTE["text"], fontsize=8.5, fontweight="bold")
        ax_bd.text(val + 0.01, y, annot, ha="left", va="center",
                   color=PALETTE["subtext"], fontsize=7.5)
...
def generate_confidence_plots(
    ...
    output_dir: str,
    single_result: Optional[Any] = None,   # ConfidenceResult for gauge/report

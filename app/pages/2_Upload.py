...
if uploaded is not None:
    ...
    with col_img:
        st.markdown(
            '<div class="section-title">Image Preview</div>', unsafe_allow_html=True
        )
        ...
        if show_channels:
            ...
            for ch_col, channel, name, idx in zip(
                ch_cols, [r, g, b], ["Red", "Green", "Blue"], [0, 1, 2]
            ):
    ...
    with col_info:
        ...
        st.markdown(
            f'<div class="medical-card">{info_html}</div>', unsafe_allow_html=True
        )

        arr = orig_rgb.astype(np.float32)
        st.markdown(
            '<div class="section-title" style="margin-top:1rem;">Pixel Statistics</div>',
            unsafe_allow_html=True,
        )
        pix_html = (
            stat_row(
                "Mean R,G,B",
                f"({arr[:, :, 0].mean():.1f}, {arr[:, :, 1].mean():.1f}, {arr[:, :, 2].mean():.1f})",
            )
            + stat_row(
                "Std R,G,B",
                f"({arr[:, :, 0].std():.1f}, {arr[:, :, 1].std():.1f}, {arr[:, :, 2].std():.1f})",
            )
        ...
        st.markdown(
            f'<div class="medical-card">{pix_html}</div>', unsafe_allow_html=True
        )
        ...
        for i, (ch, col) in enumerate(COLORS.items()):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    fill="tozeroy",
                    fillcolor=col.replace("#", "rgba(")
                    + ",0.08)".replace("rgba(", "rgba(")
                    if False
                    else f"rgba({int(col[1:3], 16)},{int(col[3:5], 16)},{int(col[5:7], 16)},0.08)",
                )
            )
        fig.update_layout(
        ...
        st.markdown(
            '<div class="section-title" style="margin-top:1rem;">Channel Histogram</div>',
            unsafe_allow_html=True,
        )

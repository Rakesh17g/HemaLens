...
from app.components.styles import (
    inject_css, section_title, stat_row, fancy_hr,
    risk_badge, pred_pill, confidence_bar, page_header, ICONS,
)
from app.components.model_utils import (
    init_session, has_image, run_prediction, checkpoint_exists,
)
...
risk_color = {"LOW": "#8fcdb7", "MEDIUM": "#c4956a", "HIGH": "#dc443c"}.get(risk, "#888888")
...
with diag_col:
    ...
    for col, label, val, color, sub in [
        ...
        (k2, "PROBABILITY", f"{prob*100:.1f}%",  pred_color, f"threshold {thresh:.2f}"),
  ...
  {"" if not sub else f'<div class="hl-kpi-sub">{sub}</div>'}
    ...
    breakdown_html = (
        ...
        (confidence_bar(mc_sc, f"MC Dropout (T={mc_p})") if mc_p > 0
         else f'<div style="font-size:0.75rem;color:var(--muted);margin:0.4rem 0;">MC Dropout disabled (set passes > 0)</div>') +
        confidence_bar(conf, "Combined Confidence")
    )
    st.markdown(f'<div class="medical-card">{breakdown_html}</div>', unsafe_allow_html=True)
...
with left_col:
    st.markdown('<div class="section-title">Confidence Radar</div>', unsafe_allow_html=True)
    components = ["Boundary", "Entropy", "MC Dropout", "Combined"]
    values     = [b_sc, e_sc, mc_sc if mc_p > 0 else 0.0, conf]
    radar = go.Figure(go.Scatterpolar(
        r=values + [values[0]], theta=components + [components[0]],
        fill="toself", line=dict(color="#a5f9ef", width=1.5),
        fillcolor="rgba(165,249,239,0.08)", name="Confidence",
    ))
    radar.update_layout(
        polar=dict(
            bgcolor="#0e0e0e",
        ...
        margin=dict(l=30, r=30, t=30, b=30), showlegend=False, height=240,
    )
    ...
    notes_html = (
        ...
        stat_row("Weights",   "0.35 · 0.35 · 0.30" if mc_p > 0 else "0.50 · 0.50 (MC off)")
    )
...
with right_col:
    ...
    gauge = go.Figure(go.Indicator(
        ...
        number=dict(suffix="%", font=dict(size=26, color=pred_color)),
        gauge=dict(
            axis=dict(range=[0, 100], tickcolor="#555", tickfont=dict(color="#555")),
            bar=dict(color=pred_color),
            bgcolor="#141414", borderwidth=0,
            steps=[
                dict(range=[0, thresh * 100],  color="rgba(143,205,183,0.05)"),
                dict(range=[thresh * 100, 100], color="rgba(220,68,60,0.05)"),
            ],
            threshold=dict(
                line=dict(color="#c4956a", width=2),
                thickness=0.8, value=thresh * 100,
            ),
        ),
    ))
    gauge.update_layout(
        paper_bgcolor="#0e0e0e", font=dict(color="#888", family="Inter"),

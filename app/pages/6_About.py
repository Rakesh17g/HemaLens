...
""",
    ...
    st.markdown(
        f'{perf_rows}<div style="font-size:0.62rem;color:var(--muted);margin-top:0.5rem;">Reported on ALL-IDB2 public dataset. For reference only.</div></div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    '<div class="section-title">Project Structure</div>', unsafe_allow_html=True
)
structure = [
...
for ref in refs:
    st.markdown(
        f'<div style="font-size:0.78rem;color:var(--subtext);padding:0.3rem 0;border-bottom:1px solid var(--border);">{ref}</div>',
        unsafe_allow_html=True,
    )

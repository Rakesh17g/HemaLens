...
overlay_bgr = cv2.addWeighted(
    orig_bgr, 1 - overlay_alpha, colormap_bgr, overlay_alpha, 0
)
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Side-by-Side", "Heatmap", "Overlay", "Activation Profile"]
)
...
with tab1:
    ...
    c2.image(
        colormap_rgb,
        caption=f"Grad-CAM ({cam['method'].upper()})",
        use_container_width=True,
        clamp=True,
    )
    c3.image(
        overlay_rgb,
        caption=f"Overlay  α={overlay_alpha:.2f}",
        use_container_width=True,
        clamp=True,
    )
    st.markdown(
...
with tab2:
    ...
    with col_cb:
        st.markdown(
            '<div class="section-title">Activation Info</div>', unsafe_allow_html=True
        )
        hw = heatmap
        info = (
...
with tab3:
    st.image(
        overlay_rgb,
        caption=f"Grad-CAM Overlay · α={overlay_alpha:.2f}",
        use_container_width=True,
        clamp=True,
    )
...
with tab4:
    st.markdown(
        '<div class="section-title">Activation Profile</div>', unsafe_allow_html=True
    )
    ...
    fig2.update_layout(
        ...
        plot_bgcolor="#050505",
        font={"color": "#888", "family": "Inter"},
        legend={"bgcolor": "#0e0e0e", "bordercolor": "rgba(255,255,255,0.07)"},
        xaxis={"gridcolor": "rgba(255,255,255,0.04)", "title": "Pixel position"},
        yaxis={"gridcolor": "rgba(255,255,255,0.04)", "title": "Mean Activation"},
    ...
    st.markdown(
        '<div class="section-title" style="margin-top:1rem;">Activation Surface (3D)</div>',
        unsafe_allow_html=True,
    )
    fig3 = go.Figure(
        go.Surface(

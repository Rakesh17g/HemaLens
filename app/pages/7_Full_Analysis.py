...
from app.components.styles import (
    inject_css, section_title, stat_row, risk_badge,
    pred_pill, confidence_bar, ICONS, page_header,
)
from app.components.model_utils import (
    init_session, run_full_pipeline, generate_pdf_bytes,
    checkpoint_exists, get_device,
)
...
steps_html = "".join([
    ...
    f'</div>'
    for num, label in [
        ("01", "Upload"), ("02", "Preprocess"), ("03", "Inference"),
        ("04", "Confidence"), ("05", "Grad-CAM"), ("06", "Report"),
    ]
])
st.markdown(f"""
...
# ─── Step 1: Upload ────────────────────────────────────────────────────────────
st.markdown('<div class="section-title">01 — Upload Cell Image</div>', unsafe_allow_html=True)
...
if raw_bytes:
    ...
    with prev_col:
        st.image(thumb, caption=st.session_state.get("uploaded_filename", ""), use_container_width=True)
    with btn_col:
        config_html = (
            ...
            stat_row("Grad-CAM",  f"{st.session_state['gradcam_method'].upper()} · layer {st.session_state['gradcam_layer']}")
        )
        st.markdown(f'<div class="medical-card">{config_html}</div>', unsafe_allow_html=True)
    ...
    if run_btn or st.session_state.get("pipeline_result"):
        ...
        st.markdown('<div class="section-title">02 — Diagnostic Results</div>', unsafe_allow_html=True)
        ...
        risk_color = {"LOW": "#8fcdb7", "MEDIUM": "#c4956a", "HIGH": "#dc443c"}.get(risk, "#888")

        k1, k2, k3, k4 = st.columns(4)
        for col, label, val, color, sub in [
            (k1, "PREDICTION",  pred,               pred_color, ""),
            (k2, "PROBABILITY", f"{prob*100:.1f}%", pred_color, f"threshold {result['threshold']:.2f}"),
            (k3, "CONFIDENCE",  f"{conf*100:.1f}%", "#a5f9ef",  "model certainty"),
            (k4, "RISK",        risk,               risk_color, {"LOW": "Trustworthy", "MEDIUM": "Review advised", "HIGH": "Human review required"}.get(risk, "")),
        ]:
            col.markdown(f"""
<div class="hl-kpi hl-pulse">
  ...
  {"" if not sub else f'<div class="hl-kpi-sub">{sub}</div>'}
        ...
        # ── Grad-CAM ─────────────────────────────────────────────────────────
        st.markdown('<hr class="fancy-hr">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">03 — Grad-CAM Attention Map</div>', unsafe_allow_html=True)
        ...
        img_c2.image(result["colormap"], caption=f"Attention ({result['gradcam_method'].upper()})", use_container_width=True, clamp=True)
        ...
        st.markdown(f'<div class="medical-card">{hm_stats}</div>', unsafe_allow_html=True)

        # ── Confidence breakdown ──────────────────────────────────────────────
        st.markdown('<hr class="fancy-hr">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">04 — Confidence Breakdown</div>', unsafe_allow_html=True)
        ...
        with left:
            bars = (
                ...
                (confidence_bar(mc_sc, f"MC Dropout (T={mc_p})") if mc_p > 0
            ...
            detail = (
                ...
                stat_row("Weights",  " · ".join(f"{v:.2f}" for v in result["weights"].values()))
            )
            st.markdown(
        ...
        with right:
            ...
            radar = go.Figure(go.Scatterpolar(
                ...
                fill="toself", line=dict(color="#a5f9ef", width=1.5),
                fillcolor="rgba(165,249,239,0.08)",
            ))
            radar.update_layout(
                polar=dict(
                    bgcolor="#0e0e0e",
                    radialaxis=dict(range=[0, 1], tickfont=dict(size=8, color="#555"),
                                    gridcolor="rgba(255,255,255,0.05)", linecolor="rgba(255,255,255,0.05)"),
                    angularaxis=dict(tickfont=dict(size=10, color="#888"),
                                     gridcolor="rgba(255,255,255,0.05)", linecolor="rgba(255,255,255,0.05)"),
                ),
                paper_bgcolor="#050505", font=dict(color="#888", family="Inter"),
                margin=dict(l=30, r=30, t=20, b=20), showlegend=False, height=260,
            )
            st.plotly_chart(radar, use_container_width=True)

        # Clinical guidance banner
        bg_map  = {"LOW": "rgba(143,205,183,0.06)", "MEDIUM": "rgba(157,114,80,0.07)", "HIGH": "rgba(220,68,60,0.07)"}
        bdr_map = {"LOW": "rgba(143,205,183,0.2)", "MEDIUM": "rgba(157,114,80,0.2)", "HIGH": "rgba(220,68,60,0.2)"}
        st.markdown(f"""
<div style="background:{bg_map.get(risk,'rgba(255,255,255,0.03)')};
  ...
  <div style="font-size:0.82rem;color:var(--subtext);line-height:1.6;">{note}</div>
        ...
        # ── PDF Export ────────────────────────────────────────────────────────
        st.markdown('<hr class="fancy-hr">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">05 — Export Report</div>', unsafe_allow_html=True)
            ...
            with st.spinner("Generating PDF…"):
                try:
                    pdf_bytes = generate_pdf_bytes(
                        ...
                        analyst       = st.session_state.get("analyst",       "AI Diagnostic Assistant"),
                        model_version = st.session_state.get("model_version", "EfficientNet-B0 v1.0"),
                    )
                    ...
                    import traceback
                    st.error(f"PDF generation failed! Error: {repr(e)}\n\nTraceback:\n{traceback.format_exc()}")

        with info_col:
            st.markdown('<div class="section-title">Report Contents</div>', unsafe_allow_html=True)
            contents = [
                ("Original Image",       f"224×224 blood-smear (Wright-Giemsa)"),
                ("Grad-CAM Heatmap",     f"MAGMA overlay · {result['gradcam_method'].upper()} · layer {result['gradcam_layer']}"),
                ("Diagnosis Panel",      f"{pred} · {prob*100:.1f}% · {conf*100:.1f}% confidence"),
                ("Confidence Scores",    "Boundary · Entropy · MC Dropout"),
                ("Model Details",        f"{st.session_state.get('model_version','EfficientNet-B0 v1.0')}"),
            ...
            st.markdown(f'<div class="medical-card">{rows}</div>', unsafe_allow_html=True)

    ...
    if curves:
        fig.add_trace(
            go.Scatter(
                ...
                line={"color": A1, "width": 2},
                fill="tozeroy",
                fillcolor="rgba(165,249,239,0.06)",
            )
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                name="Youden's J",
                marker={"color": A3, "size": 10, "symbol": "diamond"},
            )
        )
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name="Random",
            line={"color": BDR, "dash": "dash", "width": 1},
        )
    )
    fig.update_layout(
        **_layout(380),
...
with tab_pr:
    ...
    if curves:
        fig.add_trace(
            go.Scatter(
                ...
                line={"color": A2, "width": 2},
                fill="tozeroy",
                fillcolor="rgba(220,68,60,0.06)",
            )
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name="Prevalence",
            line={"color": BDR, "dash": "dot", "width": 1},
        )
    )
    fig.update_layout(
        **_layout(380),
        title={"text": "Precision-Recall Curve", "font": {"color": SUB, "size": 12}},
        xaxis={"title": "Recall", "gridcolor": GRID, "range": [0, 1]},
        yaxis={"title": "Precision", "gridcolor": GRID, "range": [0, 1.02]},
    )
    st.plotly_chart(fig, use_container_width=True)
...
with tab_cm:
    ...
    fig.update_layout(
        **_layout(380),
        title={"text": "Confusion Matrix", "font": {"color": SUB, "size": 12}},
        xaxis={"side": "top"},
    )
    st.plotly_chart(fig, use_container_width=True)
...
with tab_thr:
    if curves:
        ...
        fig = go.Figure()
        for arr, name, col in [  # type: ignore
            (f1_arr, "F1 Score", A1),
            (sens_arr, "Sensitivity", A3),
            (spec_arr, "Specificity", "#888"),
        ]:
            fig.add_trace(
                go.Scatter(
                    x=thresholds,
        ...
        fig.add_vline(
            ...
            annotation_font={"color": A3, "size": 10},
        )
        fig.update_layout(
            **_layout(380),
            title={
...
with tab_detail:
    ...
    for i, (group, keys) in enumerate(metric_groups.items()):
        ...
        rows = "".join(
            stat_row(
                k,
                f"{metrics.get(k, 'N/A'):.4f}"
                if isinstance(metrics.get(k), float)
                else str(metrics.get(k, "N/A")),
            )
            for k in keys
        )
        col.markdown(

...
pages = {
    "Navigation": [
        ...
        st.Page("pages/4_Explainability.py", title="Explainability", icon=":material/target:"),
        st.Page("pages/5_Metrics.py", title="Metrics", icon=":material/analytics:"),
        st.Page("pages/7_Full_Analysis.py", title="Full Analysis", icon=":material/account_tree:"),
...
<div class="hl-sidebar-brand" style="display:flex;align-items:center;gap:10px;padding:12px 18px 8px;">
  ...
  <div>
    ...
    st.markdown('<div class="hl-nav-section" style="margin-left:0.5rem;font-size:0.58rem;color:#777;letter-spacing:0.14em;text-transform:uppercase;padding:12px 0 8px;">Navigation</div>', unsafe_allow_html=True)
    ...
    st.page_link("pages/4_Explainability.py", label="Explainability", icon=":material/target:")
    st.page_link("pages/5_Metrics.py", label="Metrics", icon=":material/analytics:")
    st.page_link("pages/7_Full_Analysis.py", label="Full Analysis", icon=":material/account_tree:")
    st.page_link("pages/6_About.py", label="About", icon=":material/info:")

    # --- Model Config ---
    st.markdown('<div class="hl-nav-section" style="margin-left:0.5rem;font-size:0.58rem;color:#777;letter-spacing:0.14em;text-transform:uppercase;border-top:1px solid rgba(255,255,255,0.07);padding-top:16px;margin-top:12px;">Model Config</div>', unsafe_allow_html=True)

    st.session_state["checkpoint_path"] = st.text_input(
        "Checkpoint", value=st.session_state["checkpoint_path"],
        label_visibility="collapsed", placeholder="Path to .pth file"
    )
    st.session_state["threshold"] = st.slider(
        "Decision threshold", 0.20, 0.80,
        value=float(st.session_state["threshold"]), step=0.01
    )
    st.session_state["mc_passes"] = st.selectbox(
        "MC Dropout passes", [0, 10, 20, 50],
        index=[0, 10, 20, 50].index(st.session_state["mc_passes"])
        if st.session_state["mc_passes"] in [0, 10, 20, 50] else 0
    )
    
    st.markdown('<hr class="hl-nav-divider">', unsafe_allow_html=True)
    st.markdown('<div class="hl-nav-section" style="margin-left:0.5rem;font-size:0.65rem;color:#888;letter-spacing:0.1em;text-transform:uppercase;">Report Metadata</div>', unsafe_allow_html=True)
    st.session_state["patient_id"]    = st.text_input("Patient ID", value=st.session_state.get("patient_id", "ANON"), label_visibility="collapsed")
    st.session_state["sample_id"]     = st.text_input("Sample ID", value=st.session_state.get("sample_id", "N/A"), label_visibility="collapsed")
    st.session_state["analyst"]       = st.text_input("Analyst", value=st.session_state.get("analyst", "AI Assistant"), label_visibility="collapsed")
    st.session_state["institution"]   = st.text_input("Institution", value=st.session_state.get("institution", "HemaLens AI"), label_visibility="collapsed")
